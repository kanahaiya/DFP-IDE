...
import { XMLFormatPanel } from '@/components/tools/json-to-xml/XMLFormatPanel';
import { XMLArrayPanel } from '@/components/tools/json-to-xml/XMLArrayPanel';
import { XMLAdvancedPanel } from '@/components/tools/json-to-xml/XMLAdvancedPanel';
import { XMLPresetsPanel } from '@/components/tools/json-to-xml/XMLPresetsPanel';
import { useXMLStore } from '@/store/xml';
import { useTabs } from '@/hooks/useTabs';
import { useLayout } from '@/hooks/useLayout';
import { useResizer } from '@/hooks/useResizer';
import { convertJSONToXML } from '@/lib/xml/converter';
import { validateJSON } from '@/lib/xml/validator';
import { autoCorrectJSON } from '@/lib/autoCorrect';
import { XML_SAMPLES } from '@/data/json-to-xml-samples';
...
import { jsonToXMLHelpSections } from '@/data/json-to-xml-help';
import { jsonToXMLContent } from '@/data/json-to-xml-seo';
import { generateAllSchemas } from '@/lib/seo/schema-generator';
import type { EditorLanguage } from '@/types';

export default function JSONToXMLPage() {
  const { settings: globalSettings, updateSettings } = useXMLStore();
  ...
  const {
    ...
    maxTabs,
    canAddTab,
  } = useTabs({ toolName: 'JSON to XML', storageKey: 'xml-tabs' });
  ...
  const outputLanguage: EditorLanguage = 'xml';
  ...
  const performConversion = useCallback(() => {
    try {
      ...
      // Validate JSON input
      const validationResult = validateJSON(inputText);
      
      if (!validationResult.isValid) {
        setErrors(validationResult.errors.map(e => ({
          line: e.line,
          column: e.column,
          message: e.message,
        })));
      ...
      const result = convertJSONToXML(inputText, settings);
      ...
      if (result.success) {
        ...
        trackEvent('xml_conversion', 'tool_usage', 'json-to-xml');
      } else {
        setErrors([{
          line: 1,
          column: 1,
  ...
  const handleLoadTemplate = useCallback((template: { name: string; content: string; description: string }) => {
    ...
    // Track template load
    trackEvent('xml_template_load', 'tool_usage', template.name);
  }, [toast]);
  ...
  const handleDownload = useCallback(() => {
    ...
    const filename = `converted-${getTimestamp()}.xml`;
  ...
  const settingsSidebar = (
    <>
      ...
      <div className="sidebar-content">
        ...
        {settingsTabId === 'settings-arrays' && <XMLArrayPanel />}
  ...
  return (
    <>
      ...
      <main>
        ...
        >
          ...
          <HelpModal
            ...
            toolName="JSON to XML Converter"
            sections={jsonToXMLHelpSections}
          />
        </IDELayout>
      </main>
      ...
      <SEOContent 
        title={jsonToXMLContent.title}
        subtitle={jsonToXMLContent.subtitle}
        trustBadges={jsonToXMLContent.trustBadges}
        howToSectionTitle={jsonToXMLContent.howToSectionTitle}
        howToSteps={jsonToXMLContent.howToSteps}
        featuresSectionTitle={jsonToXMLContent.featuresSectionTitle}
        features={jsonToXMLContent.features}
        useCases={jsonToXMLContent.useCases}
        whyChooseSectionTitle={jsonToXMLContent.whyChooseSectionTitle}
        whyChoose={jsonToXMLContent.whyChoose}
        comparisonSectionTitle={jsonToXMLContent.comparisonSectionTitle}
        comparison={jsonToXMLContent.comparisonTable.rows}
        faqs={jsonToXMLContent.faqs}
        relatedTools={jsonToXMLContent.relatedTools}
        technicalSpecs={jsonToXMLContent.technicalSpecs}
      />

      {/* JSON-LD Structured Data */}
      <JsonLd data={generateAllSchemas(
        jsonToXMLContent.faqs,
        jsonToXMLContent.howToSteps,
        {
          toolName: 'JSON to XML Converter',
          toolUrl: '/json-to-xml',
          description: jsonToXMLContent.subtitle,
          featureList: jsonToXMLContent.features.map(f => f.title),
          howToTitle: 'How to Convert JSON to XML Online',
          howToDescription: 'Learn how to convert JSON to XML with our free online converter featuring presets for RSS, Sitemap, SOAP, and more',
        }
      )} />
