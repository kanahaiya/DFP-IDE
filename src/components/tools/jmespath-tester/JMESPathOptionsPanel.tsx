...
import type { JMESPathSettings, ResultFormat } from '@/lib/jmespath/types';

interface JMESPathOptionsPanelProps {
...
const DEBOUNCE_OPTIONS = [
  ...
  { value: 200, label: '200ms (AWS CLI)' },
  { value: 300, label: '300ms (Default)' },
  { value: 500, label: '500ms (Large files)' },
];

export function JMESPathOptionsPanel({ settings, updateSettings }: JMESPathOptionsPanelProps) {
  return (
      ...
      <div className="settings-group">
        <label className="settings-checkbox">
          <input
            ...
            onChange={(e) => updateSettings({ showExecutionTime: e.target.checked })}
          />
          <span className="checkbox-label">
        ...
        <p className="settings-help">Display query execution duration</p>
      </div>
