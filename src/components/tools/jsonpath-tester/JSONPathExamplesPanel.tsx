...
import type { JSONPathExample } from '@/lib/jsonpath/types';
import { JSONPATH_EXAMPLES, getJSONPathCategories } from '@/lib/jsonpath/examples';

interface JSONPathExamplesPanelProps {
  onLoadExample: (example: JSONPathExample, queryIndex?: number) => void;
}

export function JSONPathExamplesPanel({ onLoadExample }: JSONPathExamplesPanelProps) {
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const [expandedExample, setExpandedExample] = useState<string | null>(null);
  
  const categories = getJSONPathCategories();
  
  const filteredExamples = selectedCategory
    ? JSONPATH_EXAMPLES.filter(e => e.category === selectedCategory)
    : JSONPATH_EXAMPLES;
          ...
          <div key={example.id} className="example-item">
            ...
            >
              <div className="example-info">
                <i className={example.icon} style={{ color: '#3b82f6' }}></i>
                <div>
