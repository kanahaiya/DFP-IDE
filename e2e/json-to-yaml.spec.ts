...
/**
 * JSON to YAML E2E Tests - Production Ready
...
test.describe('JSON to YAML Converter', () => {
  ...
  test('format options are configurable', async ({ page }) => {
    ...
    // Check quote style select exists
    const quoteStyleSelect = page.locator('#quoteStyle');
    await expect(quoteStyleSelect).toBeVisible();
    
    // Check flow style select exists
    const flowStyleSelect = page.locator('#flowStyle');
    await expect(flowStyleSelect).toBeVisible();
    
    // Check line width input exists
    const lineWidthInput = page.locator('#lineWidth');
    await expect(lineWidthInput).toBeVisible();
  });

  test('tab manager is functional', async ({ page }) => {
    // Check tabs container exists (might be .tabs-container, .tab-bar, or similar)
    ...
    // Check add tab button exists (might be +, add-tab, etc.)
  ...
  test('SEO content sections are present', async ({ page }) => {
    ...
    if (await seoContent.isVisible({ timeout: 2000 })) {
      ...
      // Check for h1 heading in SEO section (use .seo-hero-title to avoid multiple h1 matches)
      const seoH1 = seoContent.locator('.seo-hero-title, h1').first();
      await expect(seoH1).toBeVisible();
      await expect(seoH1).toContainText(/JSON to YAML/i);
