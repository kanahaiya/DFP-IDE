...
/**
 * JSON Decoder E2E Tests
 * Tests for URL, Base64, Hex, HTML Entity, and Unicode decoding
 */
test.describe('JSON Decoder', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/json-decoder');
    await page.waitForLoadState('networkidle');
  });

  test('page loads with correct title and main UI elements', async ({ page }) => {
    await expect(page).toHaveTitle(/JSON Decoder/);
  ...
  test('settings sidebar is functional with decoding tabs', async ({ page }) => {
  ...
  test('decoding type selector is present and functional', async ({ page }) => {
    const decodingTypeSelect = page.locator('#decodingType');
    await expect(decodingTypeSelect).toBeVisible();
    
    const options = decodingTypeSelect.locator('option');
    expect(await options.count()).toBeGreaterThanOrEqual(5);
    
    await decodingTypeSelect.selectOption('url');
    await page.waitForTimeout(300);
    await decodingTypeSelect.selectOption('base64');
    await page.waitForTimeout(300);
    await decodingTypeSelect.selectOption('auto');
  });
  ...
  test('auto-detect and recursive options are present', async ({ page }) => {
    const autoDetectCheckbox = page.locator('#autoDetect');
    const recursiveCheckbox = page.locator('#recursiveDecode');
    
    await expect(autoDetectCheckbox).toBeVisible();
    await expect(recursiveCheckbox).toBeVisible();
  });

  test('output formatting options are present', async ({ page }) => {
    const optionsTab = page.locator('.sidebar-tab').filter({ hasText: /Options/i }).first();
    await optionsTab.click();
    await page.waitForTimeout(300);
    
    const formatCheckbox = page.locator('#formatOutput');
    const indentSelect = page.locator('#indentation');
  ...
  test('URL decoding works correctly', async ({ page }) => {
    const decodingTypeSelect = page.locator('#decodingType');
    await decodingTypeSelect.selectOption('url');
    await page.waitForTimeout(300);
    ...
    await page.keyboard.type('%7B%22test%22%3A123%7D');
  ...
  test('Base64 decoding works correctly', async ({ page }) => {
    const decodingTypeSelect = page.locator('#decodingType');
    await decodingTypeSelect.selectOption('base64');
    await page.waitForTimeout(300);
    ...
    await page.keyboard.type('eyJ0ZXN0IjoxMjN9');
  ...
  test('page is responsive', async ({ page }) => {
    ...
    const mainContent = page.locator('.ide-layout, main, .decoder-container').first();
